...
class Venue(db.Model):
  ...
  website = db.Column(db.String(120), nullable=False)#______________
  ...
  shows = db.relationship('Show', backref='venue')#________|
...
class Artist(db.Model):
  ...
  shows = db.relationship('Show', backref='artist') #_______|
...
# search for "band" should return "The Wild Sax Band".
    search = request.form['search_term']
    searched = Artist.query.filter(
        Artist.name.ilike(f'%{search}%')).all()
    data = []

    for artist in searched:
...
def edit_artist(artist_id):
  ...
  artist = Artist.query.get(artist_id)
  edit_form.name.data = artist.name
  edit_form.city.data = artist.city
  edit_form.state.data = artist.state
  edit_form.phone.data = artist.phone
  edit_form.image_link.data = artist.image_link
  edit_form.facebook_link.data = artist.facebook_link
  edit_form.website_link.data = artist.website_link
  edit_form.genres.data = artist.genres
  edit_form.seeking_venue.data = artist.seeking_venue
  edit_form.seeking_description.data = artist.seeking_description
...
def create_artist_submission():
    ...
    create = ArtistForm(request.form)
    # TODO: insert form data as a new Venue record in the db, instead
    # TODO: modify data to be the data object returned from db insertion
    error = False
    body = {}
    ...
    city = create.city.data
    state = create.state.data
    phone = create.phone.data
    genres = create.genres.data
    image_link = create.image_link.data
    facebook_link = create.facebook_link.data
    website = create.website_link.data
    venue = create.seeking_venue.data
    description = create.seeking_description.data
    try:
      artist = Artist(
        id=id,
        name=name,
